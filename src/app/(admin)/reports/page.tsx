...
function aggregateReports(
  ...
  for (const r of reports) {
    ...
    if (existing) {
      existing.regularHours += r.totalRegularHours;
      existing.overtimeHours += r.totalOvertimeHours;
      existing.totalHours += r.totalRegularHours + r.totalOvertimeHours;

      existing.count += 1;
    } else {
      map.set(key, {
        label: key,
        regularHours: r.totalRegularHours,
        overtimeHours: r.totalOvertimeHours,
        totalHours: r.totalRegularHours + r.totalOvertimeHours,
        count: 1,
      });
    }
  }
...
function exportToExcel(reports: DailyReportWithUser[]) {
  const rows = reports.map((r) => ({
    ...
    開始: r.startTime,
    終了: r.endTime,
    所定時間: r.totalRegularHours,
    残業時間: r.totalOvertimeHours,
    合計時間: r.totalRegularHours + r.totalOvertimeHours,
    作業内容: r.workEntries
